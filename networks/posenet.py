...
class PoseExpNet(nn.Module):
    ...
    def __init__(self, nb_ref_imgs=2, output_exp=False):
        ...
        self.conv1 = conv(3               , conv_channels[0], kernel_size=7)
        ...
        self.conv7 = conv(conv_channels[5], conv_channels[6])

        self.pose_pred = nn.Conv2d(conv_channels[6], 6*self.nb_ref_imgs, kernel_size=1, padding=0)

        if self.output_exp:
            upconv_channels = [256, 128, 64, 32, 16]
            self.upconv5 = upconv(conv_channels[4],   upconv_channels[0])
            self.upconv4 = upconv(upconv_channels[0], upconv_channels[1])
            self.upconv3 = upconv(upconv_channels[1], upconv_channels[2])
            self.upconv2 = upconv(upconv_channels[2], upconv_channels[3])
            self.upconv1 = upconv(upconv_channels[3], upconv_channels[4])

            self.predict_mask4 = nn.Conv2d(upconv_channels[1], self.nb_ref_imgs, kernel_size=3, padding=1)
            self.predict_mask3 = nn.Conv2d(upconv_channels[2], self.nb_ref_imgs, kernel_size=3, padding=1)
            self.predict_mask2 = nn.Conv2d(upconv_channels[3], self.nb_ref_imgs, kernel_size=3, padding=1)
            self.predict_mask1 = nn.Conv2d(upconv_channels[4], self.nb_ref_imgs, kernel_size=3, padding=1)
